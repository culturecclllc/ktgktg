...
export default function HistoryPage({ onBack }: HistoryPageProps) {
  ...
  const fetchArticles = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:8000/api/history/articles', {
        credentials: 'include',
      });
      ...
      <header className="bg-gray-800 border-b border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-2xl font-bold text-white">내 글</h1>
                    ...
                    {articles.map((article) => (
                      ...
                      >
                        ...
                        <p className="text-gray-400 text-xs">
                          {formatDateShort(article.created_date)}
                        </p>
                      </motion.div>
                    ))}
                  </div>
                ...
                <div className="mb-6">
                  <div className="bg-white rounded-lg p-4 mb-4">
                    ...
                    <p className="text-gray-600 text-sm">
                      {formatDateShort(selectedArticle.created_date)}
                    </p>
                  </div>
                  
                  <div className="flex items-center justify-between mb-6">
                    <div className="text-gray-400 text-sm">
                      작성: {formatDate(selectedArticle.created_date)}
                    </div>
